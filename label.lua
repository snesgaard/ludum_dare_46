...
label.theme = {
    background = {color = {0, 0, 0.2, 0.6}},
    text = {color = {1, 1, 1}}
}

function label.get_vertical_alignment(valign, font, h)
...
function label.draw_text(text, x, y, w, h, font, opt)
    y = y + label.get_vertical_alignment(opt.valign, font, h)

    gfx.setColor(unpack(opt.color or label.theme.text.color))
    gfx.setFont(font)
    gfx.printf(text, x+2, y, w-4, opt.align or "center")
end

function label:get_shape(text, w, opt)
...
function label:__call(text, x, y, w, h, opt)
    opt = opt or {}
    text = text or ""
    local font = opt.font or font(12)
    ...
    if not opt.hide_background then
        local color = opt.backgound_color or label.theme.background.color
    ...
    label.draw_text(text, x + margin.x, y + margin.y, w, h, font, opt)
    if opt.title then
        local title_opt = {
            color = opt.color
        }
